...
while (true)
{
    ...
    switch (move)
    {
        ...
        case 's':
            curY += 1;
            if (curY > Console.BufferHeight - 1)
            {
                curY = Console.BufferHeight - 1;
            }
            break;
        case 'D':
        case 'd':
            curX += 2;
            if (curX > Console.BufferWidth - 2)
            {
                curX = Console.BufferWidth - 2;
            }
            break;
    }
}
...
while (true)
{
    int playerAttack = r.Next(8, 16);
    int damage = (playerAttack - monsterDefence) < 0 ? 0 : playerAttack - monsterDefence;
    monsterHealth -= damage;
    if (monsterHealth <= 0)
    {
        break;
    }
    Console.WriteLine("开始战斗，攻击力为{0},怪物的防御力为{3},对怪物造成的伤害为{1},怪物还剩{2}血。",
        playerAttack,damage,monsterHealth,monsterDefence);
    Console.ReadKey(true);
    ++round;
}
Console.WriteLine("战斗结束共用{0}回合", round);
*/
#endregion
